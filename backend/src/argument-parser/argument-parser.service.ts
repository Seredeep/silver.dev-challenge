...
export class ArgumentParserService {
    ...
    public parse(
        ...
        if (
            !expectedParams.positionalArguments &&
            !expectedParams.namedArguments
        ) {
            throw new Error(
        ...
        if (expectedParams.positionalArguments) {
            expectedParams.positionalArguments.forEach(
                (expectedParam, index) => {
                    const parsedParam = posArgs.find(
                        (param) => param.position === index,
                    );
                    if (parsedParam) {
